...
class cfs_treeterm extends cfs_field
{
    ...
    function html( $field ) {
        ...
        $rand_id_prefix = md5(uniqid(rand(), true));
    ?>
        <div class="filter_posts">
            <input type="text" class="cfs_filter_input" autocomplete="off" placeholder="<?php _e( 'Search terms', 'cfs' ); ?>" />
        </div>

        <div class="available_posts post_list cfs-accordion">
        <?php foreach ( $post_top_lvl_array as $top_index => $term_top_lvl ) {
            $top_id = "lvl-1-" . $top_index;
            $group_id = "group-" . $term_top_lvl->term_id . $rand_id_prefix;
            ?>
            <div>
                <input id="<?php echo $group_id; ?>" type="checkbox" class="hidden-check" />
                <label for="<?php echo $group_id; ?>">
                    <span class="fa fa-angle-right"></span>
                    <?php echo apply_filters( 'cfs_treeterm_lvl_1_toggle', $term_top_lvl->name, $term_top_lvl->term_id, $field); ?>
                </label> 
                <div class="group-list">
                    <?php
                    foreach ($term_top_lvl->children as $mid_index => $term_mid_lvl) {
                        $subgroup_id = "sub-group-" . $term_mid_lvl->term_id . $rand_id_prefix;
                        ?>
                        <div>
                            <input id="<?php echo $subgroup_id; ?>" type="checkbox" class="hidden-check" />
                            <label for="<?php echo $subgroup_id; ?>">
                                <span class="fa fa-angle-right"></span>
                                <?php echo apply_filters( 'cfs_treeterm_lvl_2_toggle', $term_mid_lvl->name, $term_mid_lvl->term_id, $field); ?>
                            </label>
                            <div class="sub-group-list">
                                <?php
                                    foreach ($term_mid_lvl->children as $index => $term) {
                                        $subsubgroup_id = "sub-sub-group-" . $term->term_id;
                                        ?>
                                            <div rel="<?php echo $term->term_id; ?>" class="cfs_treeterm_item lvl-3<?php echo ( isset( $selected_posts[ $term->term_id]) ) ? ' used': ''; ?>" title="<?php echo $term->name; ?>">
                                                <?php echo apply_filters( 'cfs_term_display', $term->name, $term->term_id, $field ); ?>
                                            </div>
                                        <?php
                                    }
                                ?>
    ...
    function options_html( $key, $field ) {
        $args = [ 'public' => true ];
        $choices = apply_filters( 'cfs_field_treeterm_taxonomies', get_taxonomies( $args ) );
        ...
        (function($) {
            ...
            $(function() {
                ...
                $(document).on('click', '.cfs_treeterm .selected_posts .remove', function() {
                    ...
                    parent.find('.available_posts div[rel='+term_id+']').removeClass('used').addClass('unused');
                    div.remove();
                    redraw_on_off_cells();
                    update_term_values(parent);
                });
